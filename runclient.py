...
if __name__ == "__main__":
    ...
    redispar = subparsers.add_parser("redis", help="Redis operations")
    redispar.add_argument(
        "-d", "--delete", help="delete all failed proxies", action="store_true"
    )
    redispar.add_argument("-s", "--stat", help="Stat of redis", action="store_true")

    filepar = subparsers.add_parser("file", help="Load or dump proxy file")
    filepar.add_argument(
        "-l", "--load", help="Load from file with 1 proxy per line", action="store_true"
    )
    filepar.add_argument(
        "-d", "--dump", help="Dump good proxies to a file", action="store_true"
    )
    filepar.add_argument("file", type=str, help="file path")

    invokepar = subparsers.add_parser("invoke", help="invoke 3rd party libs")
    invokepar.add_argument("lib", type=str, help="3rd party library name")

    prometheuspar = subparsers.add_parser("prometheus", help="run prometheus client")
    flaskpar = subparsers.add_parser("flask", help="run flask client")

    squidpar = subparsers.add_parser(
        "squid",
        help="Update squid conf periodically: sudo python3 runclient.py squid 60",
    )
