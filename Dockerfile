...
ENV LANG C.UTF-8
RUN apt update
RUN apt install squid -yq && sed -i 's/http_access deny all/http_access allow all/g' /etc/squid/squid.conf && service squid start && cp /etc/squid/squid.conf /etc/squid/squid.conf.backup
