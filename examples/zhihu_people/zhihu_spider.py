...
per_page = 20
info_max_process_num = 1  # 50
list_max_process_num = 1  # 10
host = 'https://www.zhihu.com'
waiting_set = 'waiting'
info_failed_set = 'info:failed'
info_success_set = 'info:success'
list_success_set = 'list:success'
list_failed_set = 'list:failed'
...
def start():
    redis_client = init()
    # 待抓取节点集合是否为空
    while not redis_client.scard(info_success_set):
        print('待抓取集合为空...')
        time.sleep(5)

    # 从待抓取节点集合随机取出一个节点
    url_token = redis_client.spop(info_success_set).decode()
    # 抓取节点代表用户的个人主页
    ...
    print('目前列表抓取成功节点集合中有 %d 人' % redis_client.scard(list_success_set))
    print('目前列表抓取失败节点集合中有 %d 人' % redis_client.scard(list_failed_set))

    print('将该用户 %s 放入列表抓取成功节点集合' % url_token)
    redis_client.sadd(list_success_set, url_token)
    print('该用户关注列表抓取完毕')
    print('')
